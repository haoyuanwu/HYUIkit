...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    if (metadataObjects.count>0) {
        ...
        if (metadataObject.type == AVMetadataObjectTypeFace) {
            ...
            BOOL isFace = CGRectContainsRect(_faceDetectionFrame, faceRegion);
            NSLog(@"是否包含头像：%d, facePathRect: %@, faceRegion: %@",isFace,NSStringFromCGRect(_faceDetectionFrame),NSStringFromCGRect(faceRegion));
...
//               CGImageRef cgImageRef = [temporaryContext createCGImage:ciImage fromRect:faceFeature.bounds];
               CGImageRef cgImageRef = [temporaryContext createCGImage:ciImage fromRect:rect];

               //resultImg即为获得的人脸图片
               UIImage   *resultImg = [[UIImage alloc] initWithCGImage:cgImageRef scale:0.1 orientation:UIImageOrientationUp];
               self->_model = [SSCardModel new];
               self->_model.image = resultImg;
...
//            _model = [SSCardModel new];
            ...
            if (_model){
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (self.delegate && [self.delegate respondsToSelector:@selector(SSScanningControllerPopController:model:)]) {
                        [self.delegate SSScanningControllerPopController:self model:self->_model];
                    }
                });
            }
        }
