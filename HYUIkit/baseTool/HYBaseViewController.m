...
+ (void)insertNotifications:(NSDate *)date alert:(NSString *)alert key:(NSString *)key{
    ...
    for (int i = 0; i<3; i++) {
        ...
        if (time<0) {
            ...
            [HYBaseViewController registerLocalNotification:0 title:[alert stringByAppendingString:days[i]] date:newDate key:[key stringByAppendingString:days[i]]];
        }
    }
}
...
+ (void)registerLocalNotification:(NSInteger)alertTime title:(NSString *)title date:(NSDate *)date key:(NSString *)key{
    ...
    [HYBaseViewController cancelLocalNotificationWithKey:key];
